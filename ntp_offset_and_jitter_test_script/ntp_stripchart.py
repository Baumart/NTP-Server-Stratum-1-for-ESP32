...
def draw_stripchart(buffers, max_offset):
    ...
    for server, label in SERVERS.items():
...
def monitor():
    ...
    while time.time() - start_time < DURATION:
        ...
        clear_screen()
        print(f"⏱️  Elapsed: {elapsed:.0f}s / {DURATION}s | Samples: {sample_count}")
        print()
    ...
    print(f"⚠️  Total spikes: {spike_count}")
